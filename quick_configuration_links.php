...
class PluginConfigurationLink {
   ...
   */
	...
	function find_by_reflection(){
		...
		foreach ($items as $topmenu => $item){
			...
			foreach ($item as $subitem){
				...
				$handler_info = $this->get_first_item($hook_data);
				//Take the first handler
				$handler_info = $this->get_first_item($handler_info);
				...
				try {
					...
					} else if ( class_exists('Closure', false) && ($handler instanceof Closure) ) {
				...
				//BUG: Not all menus with hooks are from plugins. Themes and core also have them.
	 ...
	 * @return mixed
	 */
	private function get_first_item($array) {
		//Doing it this way to support ArrayAccess.
